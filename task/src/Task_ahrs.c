...
float Angle_Z_Temp[2]={0};		//云台yaw轴校正用
float Temp_Z = 0;
...
//	Angle.Z = 	atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;		//感觉没有价值，注掉
	//求解欧拉角
//	Angle.X = atan2(2 * q2q3 + 2 * q0q1, -2 * q1q1 - 2 * q2q2 + 1) * 57.3f;
//	Angle.Y = asin(-2 * q1q3 + 2 * q0q2) * 57.3f;
//	Angle.Z = atan2(2 * q1*q2 + 2 * q0*q3, -2 * q2*q2 - 2 * q3*q3 + 1)	* 57.3f;
	Temp_Z = atan2(2 * q1_yaw*q2_yaw + 2 * q0_yaw*q3_yaw, -2 * q2_yaw*q2_yaw - 2 * q3_yaw*q3_yaw + 1)	* 57.3f;
//	Angle.Z = Angle.Z *0.439497f/389*360;
	Angle.Z = Temp_Z/772.5f*360*360/350/25*360/354*360;
	
//	Angle_Z_Temp[0] = Angle_Z_Temp[1];
//	Angle_Z_Temp[1] = Temp_Z;
//	if(abs(Angle_Z_Temp[1]-Angle_Z_Temp[0])<=300)//跳变沿数据舍弃
//	{
//		Angle.Z += (Angle_Z_Temp[1]-Angle_Z_Temp[0]);
//	}
//	if(Angle.Z-360>=0.1f)
//		Angle.Z -= 360;
//	else if(Angle.Z+360<=-0.1f)
//		Angle.Z += 360;
	
	Angle.X = atan2(2 * q2_yaw*q3_yaw + 2 * q0_yaw*q1_yaw, -2 * q1_yaw*q1_yaw - 2 * q2_yaw*q2_yaw + 1) * 57.3f;
	Angle.X = Angle.X *0.439497f/389*360;
	Angle.Y = asin(-2 * q1_yaw*q3_yaw + 2 * q0_yaw*q2_yaw) * 57.3f;
	Angle.Y = Angle.Y *0.439497f/389*360;
